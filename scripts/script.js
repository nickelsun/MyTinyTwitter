...
class Twitter {
  ...
  setAvatarOfLoginnedUser(avatarElems, avatarPathCss) {
    // If the user has no avatar, the default one will be assigned (from html)
    if (avatarPathCss) {
      const avatars = [];
    ...
    sortPost.forEach(({ id, userName, nickname, postDate, text, img, likes, liked, getDate }) => {
      this.elements.listElem.insertAdjacentHTML('beforeend', `
        <li>  
          <article class="tweet">
            <div class="row">
              <img class="avatar" src="images/${nickname}.jpg" alt="Avatar of user ${nickname}">
          ...
          `<figure class="tweet-post__image">
                        <img src="${img}" alt="Image for ${nickname}'s post.">
                      </figure>`
          : ' '
        }
                </div>
  ...
  addTweet({ text, img, submit }) {
    ...
    imgElem.addEventListener('click', () => {
      imgUrl = prompt('Enter your image URL');
    });
  }
...
const twitter = new Twitter({
  ...
  tweetElems: [
    ...
    // One more tweetElem, this time not modal. 
    // querySelector takes the value of the date field from the beginning of the page
    {
